array = "Four score and seven years ago our fathers brought
 ...
 result = array.split.inject(Hash.new(0)) { |h,v| h[v] += 1; h }

 puts result

def most_common(array)
  words = array.downcase.tr(",.?!",'').split(' ')
  counts = words.each_with_object(Hash.new(0)) { |e, h| h[e] += 1 }
  max_quantity = counts.values.max
  counts.select { |k, v| v == max_quantity }.keys
end

puts most_common(array)  #this value is the most used word.